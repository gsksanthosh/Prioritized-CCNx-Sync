...
CompareAction(struct ccn_schedule *sched,
    ...
    switch (data->state) {
		...
		if(pos == 0){
			if (debug >= CCNL_FINE)
				SyncNoteSimple(root, here, "Prioritizing difference set");
			SyncPrioMain(root,namesToFetch,len);
		}
		struct ccn_charbuf *name = namesToFetch->ents[pos].name;
		//SyncNoteSimple(root, here, name->buf);
                SyncStartContentFetch(root, name, data);
		//SyncStartContentFetch(root,prioFetchResp.name,data);
