...
//structures for sync priority queue
struct SyncPrioEntry{
	int index;//index in the namestofetch list
	struct SyncNameAccumEntry *nameEntry;
	struct SyncPrioEntry *next;
};
...
struct SyncPrioAccum {
   ...
   char *prefix;
   char *token;
   struct SyncPrioEntry *nameEntry;
   struct SyncPrioAccum *next;
   struct SyncPrioAccum *prev;
};

struct SyncPrioFetchResp{
	struct ccn_charbuf *name;
	struct SyncPrioAccum *head;
	struct SyncPrioAccum *scanPtr;
};
